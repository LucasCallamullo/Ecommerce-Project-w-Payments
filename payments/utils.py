...
def create_order_pending(request):
    ...
    try:
        with transaction.atomic():
            ...
            # Crear factura null hasta que hagamos un sucess para almacenar una factura acorde
            factura = None
...
def confirm_order(request, payer, order):
    """
    esta funcion nos sirve para almacenar y guardar todos los datos en la base de datos y asociarlo
    segun corresponda con cada base de datos
    ...
    try:
        with transaction.atomic():
            ...
            factura = Factura.objects.create(
                ...
                shipment_cost=order.envio.method.price,
                name_mp=mp_name,
            ...
            # Crear orden
            status = OrderStatus.objects.get(id=3)  # Orden en estado "Pendiente"
            order.status = status  # Cambiar estado a "Pendiente"
            order.factura = factura  # Asociar factura
            order.save()  # Guardar cambios en la base de datos
            
            # Vaciar el carrito tras la compra
            carrito.clear()

            return factura, "La nueva factura fue creada con exito!"
